...
async def run_tv_scan_and_buy() -> int:
    ...
    if vr_items:
        ...
        # fallback: stock-master.json TOP 200 (OOM 방지 — 354MB ohlcv 로드 제거)
        logger.warning("volume-rank 실패 → fallback stock-master TOP 200 조회")
        ...
        try:
            ...
            logger.info(f"fallback stock-master 기준 상위 200종목 ({len(scan_targets)})")
        except Exception:
