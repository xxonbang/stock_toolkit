...
### [버그픽스] 1000만원 시뮬 모드 — 헤더 누적과 개별/일별 표시 분배금 불일치 통일 (2026-06-01 16:10 KST)
- **변경 파일:** `frontend/src/pages/AutoTrader.tsx` (4곳, +investByKey 단일 소스)
- **원인:** 1000만원 복리 모드에서 (D) 표시 불일치 — 헤더 누적은 `cap / newBuys.length`(실제 자본·신규매수 기준)인데, 일별 손익(L1376)·개별 거래(L1464) 표시는 `Math.max(capAtBuy, 1000만) / group.length`(자본 1000만 바닥 고정·전체종목 기준)를 써서 ① 손실로 자본<1000만 구간, ② 누적형(보유 지속) 전략에서 헤더와 개별표시가 어긋남
- **수정:** 헤더 복리 루프에서 거래별 배분금 `investByKey[key] = perStock` 저장 → 일별 손익·개별 거래 표시가 이를 참조하도록 통일. 제외된 날(excludedDates) 등 미배분 거래만 기존 근사식 fallback으로 회귀 방지
- **검증:** `tsc --noEmit` 통과. 회전형(시간전략 등)은 `newBuys.length==group.length` & `cap==dailyCapitals[직전일]`로 기존 표시식과 수학적 동일 → 스크린샷 75/245/517/220주 불변. 손실 구간에서만 실제 자본 기준으로 정정
- **선행 진단:** 스크린샷 4종목(2026-06-01) 수치를 Python으로 1원 단위 역산 → capAtBuy=10,816,232, 6/1 손익 +11,899, 종료자본 10,828,131 전부 일치. 복리 로직 자체는 정확함을 입증
...
- **이슈:**
  1. 5/13 텔레그램으로 현대모비스(5/12)/SK하이닉스(5/11)/미래에셋증권(5/6)이 "2일 연속" AND 신호로 발송됨 — 화면에서는 종료된 신호로 분리되는데 텔레그램만 잘못 발송
  2. 화면에 `033100` 종목명이 코드 그대로 표시 (제룡전기)
- **원인:**
  1. `_calc_streak`는 "마지막 등장이 7일 이내" cutoff만 적용 → 5/13 시점에 5/6 마지막 등장도 통과. 텔레그램 발송 조건이 `and_results` 전체였음
  2. name fallback이 `combined` + `investor_data`만 조회 → 둘 다 없으면 코드를 name으로 사용 (033100 케이스)
- **내용:**
  - 텔레그램 발송: frontend `ConsecutiveSignalSection.tsx`의 freshness 기준(`last >= yesterday`)과 동일하게 KST today/yesterday 매칭만 활성 → 발송. **watch(어제 마지막 등장)도 포함**
  - name fallback: stock-master.json 매핑을 3순위 fallback으로 추가 (combined → investor_data → stock-master → code)
- **수정 과정:**
  - 1차: `last == latest_date`로 너무 엄격 → 5/13 시점 현대모비스(5/12)도 누락
  - 2차(최종): `last >= yesterday_kst`로 완화 → frontend 활성과 정확히 일치
- **검증:** py_compile OK. 5/13 시점 시뮬 — 기존 3건 발송(현대모비스+SK하이닉스+미래에셋) → 수정 후 1건(현대모비스만, watch 활성)
...
- **수정:**
  - 개별 버튼 조건 `profit_rate < 0` → `current_price > 0` (보유 종목 모두 표시)
  - 종합 계산기 진입 조건 → 보유 종목 1건 이상이면 표시, 모든 보유를 inputs에 포함
- **이름 유지:** 사용자 요청대로 "물타기 계산기" 라벨 그대로 (불타기 목적도 활용 가능)
- **계산 로직:** A탭(기본)/C탭(분할매수)은 양/음 무관 정상 동작. B탭(목표 역산)은 평단 낮추기 가정이라 양전 종목에선 A/C 우회 권장.
- **검증:** tsc + production build 통과.
