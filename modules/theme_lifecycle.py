...
def track_theme_lifecycle(theme_name: str, history: list) -> dict:
    appearances = []
    for snapshot in history:
        for theme in snapshot.get("themes", []):
            if theme.get("name") == theme_name:
                appearances.append({"date": snapshot.get("date"), "rank": theme.get("rank", 99), "leader_change": theme.get("leader_change", 0), "stock_count": theme.get("stock_count", 0)})
    if not appearances:
    ...
    return {"theme": theme_name, "stage": stage, "appeared_days": len(appearances), "current_rank": ranks[-1] if ranks else 99, "strategy": strategy_map.get(stage, ""), "history": appearances}
